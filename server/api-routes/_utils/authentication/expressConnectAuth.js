...
export const requestTokens = async () =>{
    try{
        ...
        process.env['EXPERT_CONNECT_TOKEN_EXPIRATION'] = moment().add(credentials.expires_in,"s").utc().valueOf();
...
export const refreshToken = async ()=>{
    try{
        if(moment().utc().valueOf() < process.env['EXPERT_CONNECT_TOKEN_EXPIRATION'])
            return {success:true}
