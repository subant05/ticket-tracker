...
const checkFormantToken = async (token) =>{
    try{
        const response = await fetch(`https://api.formant.io/v1/admin/users/${token}`)
...
export const packageAuthentication =  async (req, res, next) => {
    if(req.path == '/upload' 
        && req.headers.authorization != `Basic ${process.env.SOFTWARE_DEPLOYMENT_TOKEN}`){
            res.status(401)
            res.send("401 Unauthorized")
    } else if(req.path.includes("/package") && req.query.token && req.query.token == process.env.VEHICLE_TOKEN ){
        next()
    }
    ...
    // } 
        ...
        await requestAuthenciation(req, res, next)
}
...
export const formantEventAuth =  async (req, res, next) => {
    try {
        if ( req.headers.authorization !== `Basic ${process.env.FORMANT_EXPERT_WEBHOOK_AUTHORIZATION}`
            ){
                res.status(401)
                res.send(`401 Unauthorized`)
        }
...
export const expertConnect =  async (req, res, next) => {
    ...
    if (req.headers.authorization !== `Basic ${process.env.EXPERT_CONNECT_WEBHOOK_AUTHORIZATION}`
        ){
            res.status(401)
            res.send(`401 Unauthorized`)
    }
...
export const formantConnect =  async (req, res, next) => {
    ...
    if (req.headers.authorization !== `Basic ${process.env.FORMANT_WEBHOOK_AUTHORIZATION}`
        ){
            res.status(401)
            res.send(`401 Unauthorized`)
    }
