...
export const checkAssociatedStreams = async (data) => {
  ...
  try {
    const associatedStreams =  await Query.Tickets.Select.Formant.sqlSelectAssociatedStreams(data)
    assocData = await getStreams(
        data.deviceId,data.time
        , associatedStreams
          .rows
          .map(assoc=>assoc.stream_name)
    )
   rules = await Query.Tickets.Select.Formant.sqlSelectAssociatedStreamsConditions(data)
   ...
   Object.keys(assocData).forEach(key => {
    ...
    const assocDataFormatting = await Query.Tickets.Select.Formant.sqlSelectRuleTicketAssocStreamFormatting(data)
