...
export const parseVadcDiagnostics = (vadcDiagnosticsPoints) => {
  ...
  const convertedValues =  vadcValues.filter(f=>{
      return [
        "supplemental_error_code"
        , "halt_code"
        , "autonomy_state" ].indexOf(f.label) > -1
      })
      .map(r=> {
          switch(r.label){
              case "supplemental_error_code":
...
export const getFormantDeviceData = async (device_id) => {
  ...
  try {
    ...
    const response = await fetch(`${process.env.FORMANT_API_URL}/v1/admin/devices/${device_id}`, {
        method:"GET",
        headers:{
...
export const getVehicleFormantData = async ( 
  ...
  try {
    ...
    if(payload.items[1])
      formantData.vadcDiagnostics = parseVadcDiagnostics(payload.items[1].points)
