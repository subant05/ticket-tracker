...
export const updateExpertConnectTicket = async (ticketId, data) => {
  let ecTicket = null
  try {
    ...
    const response = await fetch(`${process.env.EXPERT_CONNECT_REQUEST_URL}/${process.env.EXPERT_CONNECT_COMPANY_ID}/tickets/${ticketId}`, {
        method:"PUT"
        , body:body
        , headers: headers
    });
