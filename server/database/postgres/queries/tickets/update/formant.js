...
export const sqlDeactivateTickets = async (tickets =[])=>{
  ...
  try {
    if(!tickets.length)
      throw new Error("Must include a list of tickets inorder to deactive active rules")
    ...
    console.log("FORMANT UPDATE TICKET FROM ACTIVE TO DEACTIVE ERROR", e.message)
